...
//+------------------------------------------------------------------+
//| Format datetime as ISO 8601 timestamp (e.g. 2026-02-11T06:45:00Z) |
//+------------------------------------------------------------------+
string FormatTimestampISO(datetime dt) {
   MqlDateTime mdt;
   TimeToStruct(dt, mdt);
   return StringFormat("%04d-%02d-%02dT%02d:%02d:%02dZ", mdt.year, mdt.mon, mdt.day, mdt.hour, mdt.min, mdt.sec);
}

//+------------------------------------------------------------------+
//| Build trade ID with timestamp to avoid duplicates                 |
//| Format: SYMBOL_Tnnn-2026-02-11T06:45:00Z                         |
//+------------------------------------------------------------------+
string GetTradeIdWithTimestamp(const string& signalId) {
   if(signalId == "") return "";
   string ts = FormatTimestampISO(TimeGMT());
   return signalId + "-" + ts;
}
...
double CalculateAdjustedTarget(const TradingSignal& signal, double lotSize, double actualEntryPrice) {
   ...
   // Use COMMISSION only for target adjustment (matches lot size formula - spread applies to both win/loss)
   ...
   // Expected net loss at SL = gross loss + commission (matches lot size design)
   double expectedNetLoss = (lotSize * stopDistancePips * pipValueInAccountCurrency) + totalCommission;
   
   // Target: net profit at TP = expected net loss (true 1:1 after costs)
   // Net profit = gross - commission, so: gross = expectedNetLoss + commission
   // Required TP pips: (expectedNetLoss + commission) / (lotSize * pipValue) = stopDistancePips + 2*commission/(lotSize*pipValue)
   double netProfitEdgePercent = 0.01;  // 1% edge: net profit slightly exceeds net loss
   double targetNetProfit = expectedNetLoss * (1.0 + netProfitEdgePercent);
   double requiredGrossProfit = targetNetProfit + totalCommission;
   ...
   // Cap: allow enough to cover commission (typically 1-3 pips for forex). Old 5% cap was too restrictive.
   double maxAdjustmentPips = 5.0;  // Sufficient for commission on typical forex positions
   ...
   if(costPipsToAdd > maxAdjustmentPips) {
      ...
      targetNetProfit = requiredGrossProfit - totalCommission;
   }
   
   // Ensure TP is larger than SL to guarantee net profit > net loss after commission
   double minTPPips = stopDistancePips + (2.0 * totalCommission / (lotSize * pipValueInAccountCurrency));  // Min to cover commission
   ...
   // Calculate final expected values (commission only - matches lot size formula)
   ...
   Print("TradingSignalEA: ========================================");
   Print("TradingSignalEA: TARGET ADJUSTMENT FOR COMMISSION");
   Print("TradingSignalEA: ----------------------------------------");
...
bool ExecuteTrade(const TradingSignal& signal) {
   ...
   string tradeId = GetTradeIdWithTimestamp(signal.id);
   request.comment = "Signal: " + tradeId + " | Risk: " + DoubleToString(signal.risk_percent, 2) + "%";
         ...
         if(StringFind(comment, "Signal:") >= 0 && signalId == "") {
            ...
            if(signalPos >= 0) {
               ...
               if(pipePos > signalPos) {
                  ...
                  // Fallback: broker may truncate comment; take rest of string as trade ID
